...
const UserSiteReport = () => {
  // const [todayStr, setSelectDate] = useState('');
  ...
  useEffect(() => {
    const fetchPenaltyTypes = async () => {
      try {
        ...
        const response = await axios.post('https://testingNewAPI.mplusparking.com/AppServerCall/getPenaltyTypes', {
          imei,
          emailID,
          password,
          token,
  ...
  useEffect(() => {
    const fetchPassTypes = async () => {
      try {
        ...
        const response = await axios.post('https://testingNewAPI.mplusparking.com/AppServerCall/getPassTypes', null, {
          params: {
            imei,
            emailID,
            password,
  ...
  const handleShowSales = async () => {
    try {
      ...
      //console.log('Selected Date:', todayStr);
      ...
      if (response.data && response.data.Data && Array.isArray(response.data.Data.list)) {
        ...
        list.forEach(item => {
          ...
          if (item.passstartdate && item.passstartdate.includes(todayStr)) {
          ...
          if (item.penaltytime && item.penaltytime.includes(todayStr)) {
  ...
  const renderTableRows = () => {
    ...
    const filteredPasses = salesData.filter(item => item.passstartdate && item.passstartdate.includes(todayStr));
    const filteredPenalties = salesData.filter(item => item.penaltytime && item.penaltytime.includes(todayStr));
        ...
        <Col md="8">
          <Card className='formcard'>
            <CardBody>
              ...
              {/* <FormGroup>
                <Label style={{ fontSize: '18px' }} for="todayStr">Select Date</Label>
                <Input
                  type="date"
                  id="todayStr"
                  value={todayStr}
            ...
            <Card>
              <CardBody>
                <div id="printable-receipt" ref={receiptRef} className="receipt">
                  ...
                  <h4 className="receipt-title">Sales Date: {todayStr}</h4>
